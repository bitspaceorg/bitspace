...
type teamMetaType = Array<{
	name: string;
	aka: string;
	links: Array<{ url: string }>;
	pic: { url: string, alt: string };
	quote: string;
	type: string;
}>;
...
export default function Page() {
	...
	const [teamMeta, setTeamMeta] = useState<teamMetaType | null>(null);
	const [links, setLinks] = useState<Array<string> | null>([])

	useEffect(() => {
		const GET = async () => {
			const { data } = await axios.get("/api/team/meta");
			const sortOrder = {
				"president": 1,
				"vice president": 2,
				"CCO": 3,
			...
			const d = Object.keys(data).map((i) => { return [i, data[i].type] })
				.sort((a, b) => {
					// @ts-ignore
					return (sortOrder[a[1]] - sortOrder[b[1]])
				});
			const temp = (Object.values(data) as teamMetaType).sort((a, b) => {
				// @ts-ignore
				return (sortOrder[a.type] - sortOrder[b.type]);
			});
			setLinks(d.map((i) => i[0]));
			console.log(d.map((i) => i[0]));
			...
			<main className="flex flex-col justify-center items-center pt-16 lg:px-8 lg:pt-5 bg-cream">
				<section className="hscreen flex flex-col w-full lg:w-[900px] px-4 select-none">
					...
					<section className="flex flex-col space-y-2">
						{teamMeta?.filter(i => i.type != "member").map((member, index) => (
							<Team
								key={index}
					...
					<section className="grid grid-cols-2 lg:grid-cols-4 space-y-5 py-10">
						{teamMeta?.filter(i => i.type == "member").map((member, index) => (
							<GeneralTeam key={index} href={links ? links[teamMeta?.filter(i => i.type != "member").length + index] : ""} links={member.links} pic={member.pic} name={member.name} />
						))}
					</section>
				</section>
